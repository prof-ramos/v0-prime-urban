...
export const restrictedIpAccess = (allowedIps: string[]): Access => {
  return ({ req: { headers } }) => {
    ...
    // This example only supports exact IPs (no CIDR parsing).
...
export const timedAsyncAccess: Access = async ({ req }) => {
  ...
  const result = await fetch(`${authServiceURL}/validate`, {
    headers: {
...
const checkOrgStatus = async (orgId: string): Promise<boolean> => {
  // Example: call a service or cache layer to validate org status
  return Boolean(orgId)
}

// ✅ Fast: cache async lookup in request context
export const fastAccess: Access = async ({ req: { user, context } }) => {
  if (!user?.orgId) return false

  if (typeof context.orgStatus !== 'boolean') {
    context.orgStatus = await checkOrgStatus(String(user.orgId))
  }

  return Boolean(context.orgStatus)
}
```
