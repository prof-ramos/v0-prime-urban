...
export const hasMinimumRole = (minRole: string): Access => {
  const roleHierarchy = ['user', 'editor', 'admin']

  return ({ req: { user } }) => {
    const userHighestRole = Math.max(
      ...(user?.roles?.map((role) => roleHierarchy.indexOf(role)) || [-1]),
    )
...
const preventSelfRoleChange: FieldAccess = ({ req: { user }, id }) => {
  ...
  // Non-admin users can update roles only for other users (example rule)
  return true
}
